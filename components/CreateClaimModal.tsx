...
import { Claim, ClaimSeverity, ClaimType, ClaimConfirmation, User, TraceabilityAnalysis, RootCauseAnalysis, FishboneAnalysisData, Attachment, FilterStatus } from '../types';
...
export const CreateClaimModal: React.FC<CreateClaimModalProps> = ({ onClose, onCreateClaim }) => {
    ...
    const [formData, setFormData] = useState({
        claimType: ClaimType.Customer,
        confirmation: ClaimConfirmation.Pending,
        customerName: '',
        discoveredByDepartment: 'QC',
        orderId: '',
        productCode: '',
        defectType: '',
        quantity: 0,
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col">
                <div className="flex justify-between items-center p-4 border-b dark:border-gray-700 flex-shrink-0">
                    <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100">{t('claims.createClaim')} (8D Report)</h2>
                ...
                <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden">
                    <div className="p-6 overflow-y-auto">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="md:col-span-2 font-semibold text-ykk-blue">D1: {t('claims.generalInfo')}</div>
                            <InputField label={t('claims.claimType')} id="claimType">
                                <select id="claimType" name="claimType" value={formData.claimType} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-ykk-blue focus:border-ykk-blue bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200">
                                    {Object.values(ClaimType).map(type => <option key={type} value={type}>{type}</option>)}
                                </select>
                            </InputField>
                            {formData.claimType === ClaimType.Customer ? (
                                <InputField label={t('claims.customerName')} id="customerName">
                                ...
                                <InputField label={t('claims.discoveredByDepartment')} id="discoveredByDepartment">
                                    <select id="discoveredByDepartment" name="discoveredByDepartment" value={formData.discoveredByDepartment} onChange={handleChange} required className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-ykk-blue focus:border-ykk-blue bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200">
                                        {DEPARTMENTS.map(dep => <option key={dep} value={dep}>{dep}</option>)}
                                    </select>
                                </InputField>
                            )}
                            <InputField label={t('claims.orderId')} id="orderId">
                            ...
                            <InputField label={t('claims.productCode')} id="productCode">
                            ...
                            <InputField label={t('claims.completedPrs')} id="completedPrs">
                            ...
                            <InputField label={t('claims.responsibleDepartment')} id="responsibleDepartment">
                             ...
                             <InputField label={t('claims.assignee')} id="assigneeId">
                            ...
                            <InputField label={t('claims.deadline')} id="deadline">
                            ...
                            <InputField label={t('claims.errorCausedBy')} id="errorCausedBy">
                                <input type="text" id="errorCausedBy" name="errorCausedBy" value={formData.errorCausedBy} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-ykk-blue focus:border-ykk-blue bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200" placeholder={t('claims.errorCausedByPlaceholder')}/>
                            </InputField>
                            <InputField label={t('claims.errorCausedById')} id="errorCausedById">
                                <input type="text" id="errorCausedById" name="errorCausedById" value={formData.errorCausedById} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-ykk-blue focus:border-ykk-blue bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200" placeholder={t('claims.errorCausedByIdPlaceholder')}/>
                            </InputField>
                             <div className="md:col-span-2 mt-4 font-semibold text-ykk-blue">D2: {t('claims.problemDescription')}</div>
                             <InputField label={t('claims.defectType')} id="defectType">
                             ...
                             <InputField label={t('claims.severityLevel')} id="severity">
                                <select id="severity" name="severity" value={formData.severity} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-ykk-blue focus:border-ykk-blue bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200">
                                    {Object.values(ClaimSeverity).map(s => <option key={s} value={s}>{t(`severity.${s.toLowerCase()}`)}</option>)}
                                </select>
                            </InputField>
                             <InputField label={t('claims.defectQuantity')} id="quantity">
                             ...
                             <InputField label={t('claims.totalQuantity')} id="totalQuantity">
                            ...
                            <div className="md:col-span-2">
                                <InputField label={t('claims.discoveryLocation')} id="discoveryLocation">
                             ...
                             <div className="md:col-span-2">
                                <InputField label={t('claims.detailedDescription')} id="description">
                            ...
                            <div className="md:col-span-2">
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{t('claims.attachments')}</label>
                     ...
                     <div className="flex justify-end items-center p-4 border-t dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 rounded-b-xl flex-shrink-0">
                        <button onClick={onClose} type="button" disabled={isUploading} className="px-4 py-2 text-sm font-medium text-gray-700 bg-white dark:bg-gray-600 dark:text-gray-200 border border-gray-300 dark:border-gray-500 rounded-md shadow-sm hover:bg-gray-50 dark:hover:bg-gray-500 focus:outline-none disabled:opacity-50">{t('actions.cancel')}</button>
                        <button type="submit" disabled={isUploading} className="ml-3 px-4 py-2 text-sm font-medium text-white bg-ykk-blue border border-transparent rounded-md shadow-sm hover:bg-ykk-blue/90 focus:outline-none disabled:opacity-50 flex items-center">
                            ...
                            {isUploading ? t('claims.uploading') : t('claims.createClaim')}
                        </button>
                    </div>
                </form>
            </div>
