    ...
    return (
        <div onClick={() => onClaimSelect(claim)} className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md hover:border-ykk-blue cursor-pointer transition-all duration-200">
            <div className="flex justify-between items-start">
                ...
                <div className={`w-3 h-3 rounded-full ${getSeverityStyles(claim.severity)}`} title={`${t('common.severity')}: ${t(`severity.${claim.severity.toLowerCase()}`)}`}></div>
            </div>
...
export const ClaimsBoard: React.FC<ClaimsBoardProps> = ({ claims, onClaimSelect, onNewClaimClick, currentUser }) => {
    ...
    const statusColumns: { title: string; status: ClaimStatus }[] = [
        { title: t('status.new'), status: ClaimStatus.New },
        { title: t('status.inProgress'), status: ClaimStatus.InProgress },
        { title: t('status.pendingCustomer'), status: ClaimStatus.PendingCustomer },
        { title: t('status.completed'), status: ClaimStatus.Completed },
    ];
    
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100">{t('claims.title')}</h2>
